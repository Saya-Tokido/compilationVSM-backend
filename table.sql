...
(
    ...
    `obj_score`    int(11) DEFAULT 0 COMMENT '客观题成绩',
    `method_score` int(11) DEFAULT 0 COMMENT '函数题成绩',
    `lexer_score`  int(11) DEFAULT 0 COMMENT '词法分析题成绩',
    `create_time`  timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`  timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_delete`    tinyint(1) DEFAULT 0 COMMENT '逻辑删除标志，0为未删除，1为删除',
    PRIMARY KEY (`id`)
...
(
    ...
    `choose_score`       int(11) DEFAULT 0 COMMENT '选择题成绩',
    `fill_score`         int(11) DEFAULT 0 COMMENT '填空题成绩',
    `create_time`        timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`        timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_delete`          tinyint(1) DEFAULT 0 COMMENT '逻辑删除标志，0为未删除，1为删除',
    PRIMARY KEY (`id`)
...
(
    ...
    `rate`               decimal(6, 3) DEFAULT 0.0 COMMENT '查重率,百分比',
    `teach_class`        varchar(20)   DEFAULT '' COMMENT '查重教学班',
    `create_time`        timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`        timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `is_delete`          tinyint(1) DEFAULT 0 COMMENT '逻辑删除标志，0为未删除，1为删除',
    PRIMARY KEY (`id`)
...
(
    ...
    `lexer_pd_rate`       decimal(6, 3) DEFAULT 0.0 COMMENT '查重率阈值,百分比',
