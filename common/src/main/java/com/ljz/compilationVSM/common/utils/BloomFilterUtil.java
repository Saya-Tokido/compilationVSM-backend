...
public class BloomFilterUtil {
    ...
    public void init() {
        ...
        String luaScript = """
                return redis.call('SETBIT', KEYS[1], ARGV[1], 0)
                """;
        //false positive rate 3%
        redisUtil.stringLuaExecute(luaScript, Collections.singletonList(bloomFilterKey), 8184);
    ...
    public void add(String value) {
        ...
        List<Integer> hashCodeList = HashAlgorithmUtil.generate(value, hashSeedMap).stream().map(hashCode -> hashCode % 8192).toList();
        String luaScript = """
    ...
    public boolean exists(String value) {
        ...
        List<Integer> hashCodeList = HashAlgorithmUtil.generate(value, hashSeedMap).stream().map(hashCode -> hashCode % 8192).toList();
        String luaScript = """
                local result1 = redis.call('GETBIT', KEYS[1], ARGV[1])
                local result2 = redis.call('GETBIT', KEYS[1], ARGV[2])
                local result3 = redis.call('GETBIT', KEYS[1], ARGV[3])
